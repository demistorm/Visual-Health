...
// Samples player skins to get per-player damage tint colors
public final class SkinColorSampler {
    ...
    private static final int FALLBACK_TINT = 0xFF00009F;
        ...
        try (skin) {
            ...
            for (int y = 0; y < skin.getHeight(); y++) {
                for (int x = 0; x < skin.getWidth(); x++) {
                    int pixel = skin.getPixelRGBA(x, y);
            ...
            // Use the 2nd least common bucket (less common but not only appearing once color)
            ...
            // Pack as ABGR
            int tint = (0xFF << 24) | (outB << 16) | (outG << 8) | outR;
